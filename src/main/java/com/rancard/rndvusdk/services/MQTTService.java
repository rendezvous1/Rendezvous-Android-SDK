...
import com.rancard.rndvusdk.RndvuNotification;
...
public class MQTTService extends Service implements MqttSimpleCallback
{
    ...
    public static final String APP_ID = "com.rancard.rndvusdk";
    private static final String TAG = MQTTService.class.getName();
    private RndvuNotification rndvuNotification = null;

    // constants used to notify the Activity UI of received messages
    public static final String MQTT_MSG_RECEIVED_INTENT = "com.rancard.rndvusdk.MSGRECVD";
    public static final String MQTT_MSG_RECEIVED_TOPIC  = "com.rancard.rndvusdk.MSGRECVD_TOPIC";
    public static final String MQTT_MSG_RECEIVED_MSG    = "com.rancard.rndvusdk.MSGRECVD_MSGBODY";

    // constants used to tell the Activity UI the connection status
    public static final String MQTT_STATUS_INTENT = "com.rancard.rndvusdk.STATUS";
    public static final String MQTT_STATUS_MSG    = "com.rancard.rndvusdk.STATUS_MSG";

    // constant used internally to schedule the next ping event
    public static final String MQTT_PING_ACTION = "com.rancard.rndvusdk.PING";
    ...
    private String brokerHostName = Rendezvous.mqttBrokerUrl;
    // taken from preferences
    //    topic we want to receive messages about
    //    can include wildcards - e.g.  '#' matches anything
    private String[]  topicName  = Rendezvous.mqttReceiver;
    ...
    public void onCreate()
    {
        ...
        brokerHostName = Rendezvous.mqttBrokerUrl;
        topicName            = Rendezvous.mqttReceiver;
...
//            Notification notification = new Notification(R.drawable.abc_ab_share_pack_mtrl_alpha,
//                    "MQTT",
...
//            nm.notify(MQTT_NOTIFICATION_ONGOING, notification);
            ...
            if (isOnline())
            {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        boolean returnVal = connectToBroker();
                        if (returnVal)
                        {
    ...
    private void notifyUser(String gcmPayload)
    {
        if (gcmPayload != null) {
            if (!gcmPayload.isEmpty()) {
                Log.i(TAG, "MESSAGE RECEIVED VIA MQTT : " + gcmPayload);
                rndvuNotification = RndvuNotification.fromGCMPayload(gcmPayload);

                if (rndvuNotification != null) {
                    System.out.println("String to send rndvu handler if not is notifictn null");
//                    Intent rndvuChatHeadService = new Intent(getApplicationContext(), RndvuChatHeadService.class);
//                    startService(rndvuChatHeadService);

                    showNotification(rndvuNotification);
                }
            }
        }
    }

    public void showNotification(final RndvuNotification rndvuNotification) {
        System.out.println("String to send rndvu handler show notification");
        switch (rndvuNotification.getDisplayMode()) {
                    case TOAST:
                        showNotificationAsToast(rndvuNotification);
                        break;
                    case SYSTEM_TRAY:
                        showNotificationInSystemTray(rndvuNotification);
                        break;
                    case PROMPT:
                        showNoficationAsPrompt(rndvuNotification);
                        break;
                }

//                if(rndvuNotification.isSocial()){
//                    Rndvu.SendEventGoogleAnalytics("SDK","RndvuNotificationMQTT","Recommendation Received",context ,Rndvu.PROPERTY_ID);
//                }else{
//                    Rndvu.SendEventGoogleAnalytics("SDK","RndvuNotificationMQTT","Notification Received",context ,Rndvu.PROPERTY_ID);
//                }
                if(rndvuNotification.getAckUrl() != null){
                    if(!rndvuNotification.getAckUrl().isEmpty())
                    new RndvuNotificationAcknowledgementTask().execute(rndvuNotification);
                }

    }

    private void showNotificationAsToast(RndvuNotification rndvuNotification) {
        Toast.makeText(MQTTService.this, rndvuNotification.getMessage(), Toast.LENGTH_LONG).show();
    }

    private void showNotificationInSystemTray(RndvuNotification rndvuNotification) {
        if(rndvuNotification.getMessage().contains(RndvuNotification.ACTION_INTENT_LAUNCH_HASHING)){


            Log.i(TAG,"Message came from hash");
            Intent intent = new Intent();
            intent.putExtra("message_from_gcm",rndvuNotification.getMessage());
            intent.setAction("com.multimedia.joyonline.REFRESH_NEW_FEEDS");
...
//
//            Intent actionIntent = rndvuNotification.getEffectiveIntent(this);
//            actionIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//            PendingIntent pendingIntent = PendingIntent.getActivity(this, 0, actionIntent, Intent.FILL_IN_ACTION);
//
//            builder.setContentTitle(rndvuNotification.getTitle())
//                    .setContentText(rndvuNotification.getMessage())
//                    .setAutoCancel(true)
//                    .setDefaults(Notification.DEFAULT_ALL)
//                    .setWhen(System.currentTimeMillis())
//                    .setStyle(new NotificationCompat.BigTextStyle().bigText(rndvuNotification.getMessage()))
//                    .setSmallIcon(rndvuNotification.isSocial() ? R.drawable.ic_rndvu : Rndvu.getClient().getNonSocialRecommendationIcon())
...
//            notificationManager.notify(id, builder.build());
            ...
            Intent actionIntent = rndvuNotification.getEffectiveIntent(this);
            actionIntent.setFlags(Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT);
            PendingIntent pendingIntent = PendingIntent.getActivity(this, 0, actionIntent, Intent.FILL_IN_ACTION);



            Intent shareIntent = new Intent(Intent.ACTION_SEND);
            shareIntent.putExtra(Intent.EXTRA_SUBJECT, rndvuNotification.getTitle());
            shareIntent.putExtra(Intent.EXTRA_TEXT, String.format("Hi, I just read %s and thought you\'d be interested. Click %s to view", rndvuNotification.getTitle(), rndvuNotification.getBundle()));
            shareIntent.setType("text/plain");
            PendingIntent pendingShareIntent = PendingIntent.getActivity(context, 0, Intent.createChooser(shareIntent, "Share via..."),
                    PendingIntent.FLAG_CANCEL_CURRENT);


            Intent bookMarkReceiver = new Intent();
            bookMarkReceiver.putExtra("id", rndvuNotification.getItemId());
            bookMarkReceiver.putExtra("title", rndvuNotification.getTitle());
            bookMarkReceiver.putExtra("message", rndvuNotification.getMessage());
            bookMarkReceiver.putExtra("url", rndvuNotification.getOnClickUrl());
            bookMarkReceiver.setAction(Constants.BOOKMARK_ACTION);
            PendingIntent bookMarkIntent = PendingIntent.getBroadcast(this, 12345, bookMarkReceiver, PendingIntent.FLAG_CANCEL_CURRENT);
            ...
            builder.setContentTitle(rndvuNotification.getTitle())
                    ...
                    .setWhen(System.currentTimeMillis())
                    .setStyle(new NotificationCompat.BigTextStyle().bigText(rndvuNotification.getMessage()))
                    .setSmallIcon(rndvuNotification.isSocial() ? R.drawable.ic_rndvu : R.drawable.joy_logo)
                    .setContentIntent(pendingIntent);


            builder.addAction(R.drawable.abc_ic_menu_share_mtrl_alpha, "Share", pendingShareIntent);
            builder.addAction(R.drawable.ic_bookmark, "BookMark", bookMarkIntent);
    ...
    private void showNoficationAsPrompt(RndvuNotification rndvuNotification) {

    }

    private class RndvuNotificationAcknowledgementTask extends AsyncTask<RndvuNotification, Void, Void> {
        @Override
        ...
        @Override
        protected Void doInBackground(RndvuNotification... params) {
            RndvuNotification notification = params[0];
            if (notification != null) {
                ...
                Request request = new Request.Builder()
                        .url(notification.getAckUrl().replace("<MSISDN>",Rendezvous.msisdn))
                        .post(formBody)
                        .build();
    ...
    public void connectionLost() throws Exception
    {
        ...
        {
            connectionStatus = MQTTConnectionStatus.NOTCONNECTED_WAITINGFORINTERNET;
            Log.i(TAG, "Connection lost - no network connection");
    ...
    public void publishArrived(String topic, byte[] payloadbytes, int qos, boolean retained)
    {
        ...
        Log.i(TAG, "Message received "+topic+messageBody);
    ...
    {
        String mqttConnSpec = brokerHostName;
        ...
        catch (MqttException e)
        {
            ...
            Log.i(TAG, "Invalid connection parameters"+e.getMessage());
    ...
    private boolean connectToBroker()
    {
        ...
        catch (MqttException e)
        {
            ...
            // inform the app that we failed to connect so that it can update
            //  the UI accordingly
            Log.i(TAG, "Unable to connect "+e.getCause());
        ...
        if (isAlreadyConnected() == false)
        {
            ...
            for (String s : topics) {
                try {
                    ...
                    String[] ss = {s};
                    mqttClient.subscribe(ss, qualitiesOfService);
                ...
                if (subscribed == false) {
                    ...
                    Log.i(TAG, "Unable to subscribe to topic " + s);
                    //
                    // inform the user (for times when the Activity UI isn't running)
    ...
    public void rebroadcastReceivedMessages()
    {
        ...
        while(e.hasMoreElements())
        {
            ...
            //notifyUser(nextValue);

            dataCache.remove(nextKey);
        }
    }
